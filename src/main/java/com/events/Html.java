...
import com.events.date.Calendar.CalendarDate;

import java.io.File;
import java.util.*;

import static com.events.Html.HtmlHelper.href;
import static com.events.Html.HtmlHelper.hrefUnderline;
...
class Html {
    Set<String> outputReferences = Util.set();
    static final String FAVICON = "/favicons/favicon_l.ico";
    Dirs dirs;
    InputData inputData;

    Html(Dirs dirs, InputData inputData) {
        this.dirs = dirs;
    ...
    Map<Integer, DisplayEvent> execute(HtmlContext ctx) {
        ...
        int currentIndex = com.events.date.Calendar.indexOfCurrent();
        CalendarDate baseDate = com.events.date.Calendar.get(currentIndex + 1);
        ...
        for (int plusDays = 0; plusDays < numdays; plusDays++) {
            CalendarDate cd = com.events.date.Calendar.get(currentIndex + 1 + plusDays);
            com.events.date.Calendar.Date idate = cd.toDate();
            File ifile = datefile.get(idate);
            String data = null;
            if (ifile != null) {
                data = Util.readString(ifile.getAbsolutePath());
            }
            com.events.date.Calendar.Date date = new com.events.date.Calendar.Date(idate.dateDay, idate.dateMonth, null);
            if (isBankHoliday(date)) {
                data = null;
            }
            String weekday = WEEKDAYS_LONG.get(WEEKDAYS_ORDER.indexOf(cd.dayOfWeek));
            Util.PageData pageData;
            if ("festival".equals(ctx.type)) {
                pageData = new DataHtml(inputData).generateFestivalPageData(idate);
            } else {
                pageData = new DataHtml(inputData).generatePageData(data, ctx, baseDate);
            }
    ...
    boolean writeHtmls(HtmlContext ctx, boolean indexWritten, int plusDays, com.events.date.Calendar.Date date, String weekday, Util.Multi<String, DisplayEvent> htmlMain, Util.PageData pageData, String year) {
        String typepart = ctx.type == null ? "" : ctx.type.toLowerCase() + "/";
        ...
        if (("anywhere".equals(ctx.area))) {
            ...
            for (DisplayEvent event : pageData.events) {
                sb.append(event.title + "\t" + event.ref + "\t" + event.time + "\t" + event.link + "\n");
            }
            ...
            for (DisplayEvent event : pageData.events) {
                String status = "Y".equals(event.soldout) ? "SOLDOUT" : "AVAILABLE";
                sb.append(event.title + "\t" + event.ref + "\t" + event.time + "\t" + event.link + "\t" + status + "\n" + event.fullTitle + "\n\n");
            }
            Util.write(tsvMetaFile, sb.toString());
        }
        if (plusDays == 0 && !isBankHoliday(date)) {
            writeIndex(typepart, areapart, html);
            indexWritten = true;
        } else if (plusDays == 1 && !isBankHoliday(date) && !indexWritten) {
            writeIndex(typepart, areapart, html);
            indexWritten = true;
        } else if (plusDays == 2 && !isBankHoliday(date) && !indexWritten) {
            writeIndex(typepart, areapart, html);
        }
        return indexWritten;
    }

    void writeIndex(String typepart, String areapart, String html) {
        String path = dirs.getHtmlDir() + typepart + areapart + "index.html";
        sout("Writing: " + path);
        Util.write(path, html);
    }

    boolean isBankHoliday(com.events.date.Calendar.Date date) {
        return ("24".equals(date.dateDay) || "25".equals(date.dateDay) || "26".equals(date.dateDay)) && "dec".equals(date.dateMonth);
    }

    com.events.date.Calendar.Date extractDate(String fileName) {
        String[] split = fileName.substring(0, fileName.length() - 4).split("-");
        if (split.length < 3) return null;
        com.events.date.Calendar.Date date = new com.events.date.Calendar.Date(valueOf(parseInt(split[3])), MONTHS_ORDER.get(parseInt(split[2]) - 1), split[1]);
        com.events.date.Calendar.setDayOfWeek(date);
        return date;
    }

    String generateStartMenu(HtmlContext ctx, String datePart, String dayOfWeek) {
        String areaOrTypeToDisplay;
        if (!Util.empty(ctx.type)) {
            areaOrTypeToDisplay = ctx.type;
        } else {
            if (Util.empty(ctx.area)) {
                areaOrTypeToDisplay = inputData.getCity();
            } else {
                areaOrTypeToDisplay = ctx.area;
            }
        }
        return generateMenu(datePart, areaOrTypeToDisplay, !Util.empty(ctx.type), inputData.getCity(), dayOfWeek);
    }

    Util.Multi<String, DisplayEvent> generateHtml(String currentPageWeekday, String lhsWeekday, CalendarDate baseDate, CalendarDate currentPageDate, HtmlContext ctx, Util.PageData pageData) {
        ...
        for (int index = 0; index < numEventsToDisplay; index++) {
            ...
            if (Util.empty(event.title)) {
                continue;
            }
            outputReferences.add(event.ref);
            html.append(constructElement(Html.colorBlend, event));
            if (event.ref.equalsIgnoreCase(ctx.reference)) {
                sout(currentPageWeekday.toUpperCase() + "\t\t" + event.title);
            }
            if (index == 0) {
                html.append("      <ul class=\"xw3-ul \"" + " style=\"margin-top:0px;padding-top:0px\"" + ">\n  ");
            }
        }
        html.append("</ul></ul>\n").append(footer(false));
        return new Util.Multi<>(html.toString(), pageData.mainpageEvent);
    }

    String constructElement(List<String> colorblend, DisplayEvent event) {
        String elmt = "<a class=\"asty\" href=\"" + event.link + "\" " + "target=\"_blank\"" + ">\n" + "<li class=\"licla\">\n" + "<span class=\"cutive\">";
        ...
        String displayUnder;
        if (Util.empty(event.displayOrganizer) && Util.empty(event.district)) {
            displayUnder = "";
        } else if (!Util.empty(event.displayOrganizer) && event.displayOrganizer.contains(event.district)) {
            displayUnder = event.displayOrganizer;
        } else {
            displayUnder = event.displayOrganizer + " | " + event.district;
        }
        return elmt + "<span style=\"text-align:right;\" class=\"cutive\"><i style=\"white-space:pre;font-size:12px\">" + displayUnder + " </i></span>" + "</li>\n" + "</a>\n";
    }

    String getColorOverride(List<String> colorblend, DisplayEvent displayEvent) {
        return "background:#" + colorblend.get(Math.min(displayEvent.colorIndex, colorBlend.size() - 1));
    }
    ...
    String generateStart(String currentPageWeekday, String lhsWeekday, CalendarDate baseDate, CalendarDate currentPageDate, HtmlContext ctx, String datePart) {
        ...
        if (ctx.extended) {
            head = head + generateWeekdays(currentPageDate.dateDay, lhsWeekday, ctx.area, baseDate, 1, currentPageDate, "10", null, inputData.getCity());
        ...
        head = head + generateStartMenu(ctx, datePart, currentPageWeekday);
        return head;
    }

    String generateMenu(String datepart, String areaOrType, boolean isType, String city, String dayOfWeek) {
        String displayspaced;
        String part;
        String weekday = WEEKDAYS_LONG.get(WEEKDAYS_ORDER.indexOf(dayOfWeek)).toUpperCase();
        if ("anywhere".equals(areaOrType)) {
            displayspaced = displaySpaced("THE " + weekday + " GUIDE");
            part = datepart;
        } else if (!Util.empty(areaOrType)) {
            if (isType) {
                areaOrType = areaOrType + " " + weekday;
            }
    ...
    static String generateHeader(String area, String type, String dayOfWeek, String city) {
        String title = generateTitle((Util.empty(area) ? type : area), WEEKDAYS_LONG.get(WEEKDAYS_ORDER.indexOf(dayOfWeek)), city);
        return HtmlHeader.generate(title, FAVICON);
    }

    static String generateTitle(String areaOrType, String dayOfTheWeek, String city) {
        String title;
        if (Util.empty(city)) {
            title = "";
        } else if ("london".equals(city)) {
            if (Util.empty(areaOrType) || "anywhere".equals(areaOrType)) {
                title = "The " + uppercaseFirstLetter(dayOfTheWeek) + " Guide - Londonoo";
            } else {
                title = PhraseUtil.uppercaseAllWords(areaOrType) + " - Londonoo";
            }
        } else {
            title = uppercaseFirstLetter(city) + " | Find events for this week in " + uppercaseFirstLetter(city);
        }
        return title;
    }

    static String footer(boolean showCalendarLink) {
        String footer = "";
        if (showCalendarLink) {
            String elmt = "<a class=\"asty\" style=\"padding-top:10px;padding-right:10px;\" href=\"https://londonoo.com/west-end-musicals\">\n" + "<span class=\"interiorloc\" style=\"background:#fbfbfb\"><b>" + " West End + Musicals" + "</b></span>" + "</span>" + "\n" + "</a>\n";
    ...
    static String generateWeekdays(String selected, String currentWeekday, String area, CalendarDate baseDate, int week, CalendarDate currentPageDate, String marginbottom, String country, String city) {
        ...
        if (!Util.empty(city)) {
            if ("london".equalsIgnoreCase(city)) {
                if (!Util.empty(area)) {
                    if ("anywhere".equals(area)) {
                        citypart = "/guide/";
                    } else {
                        citypart = "/" + area.toLowerCase() + "/";
                    }
                } else {
                    citypart = "";
                }
            } else {
                citypart = "/" + city.toLowerCase() + "/";
            }
        }
        if (!Util.empty(country)) {
            citypart = "https://" + country + ".londonoo.com/";
        }
        for (int i = 0; i < 7; i++) {
            String datepart = generateDatePart(currentWeekday, i + (week * 7), baseDate);
            ...
            if (display.toLowerCase().trim().equals(selected.toLowerCase()) && currentPageDate.equals(offsetDate)) {
                ...
                sb.append("   " + "<span style=\"color:#505050;font-size:17px;\">" + href(display, citypart + datepart) + "</span>" + "  ");
            }
        }
        sb.append(" </p>");
        return sb.toString();
    ...
    static String displaySpaced(String performanceString) {
        if (performanceString == null) return null;
        performanceString = performanceString.toUpperCase();
        StringBuilder builder = new StringBuilder();
        for (char c : performanceString.toCharArray()) {
            builder.append(c + " ");
        }
        if (builder.length() > 0) {
            performanceString = builder.substring(0, builder.length() - 1);
        }
        return performanceString;
    }

    static String generateDatePart(String currentWeekday, int offset, CalendarDate baseDate) {
        List<String> weekdaysLong = HtmlHelper.nextDaysLong(currentWeekday);
    ...
    static CalendarDate generateOffsetDate(int offset, CalendarDate baseDate) {
        List<CalendarDate> calendarDays = HtmlHelper.nextDays(baseDate);
        return calendarDays.get(offset);
    }

    static class HtmlWestEndLondon {
        final Dirs dirs;
        final InputData londonInputData;

        HtmlWestEndLondon(Dirs dirs, InputData londonInputData) {
            this.dirs = dirs;
            this.londonInputData = londonInputData;
        }
        ...
        void execute() {
            ...
            for (String ref : refs) {
                refUrl.put(ref, londonInputData.getUrls(ref).get(0));
        ...
        String generate(List<String> refs, Map<String, String> refUrl, Map<String, String> refTheatre) {
            StringBuilder sb = new StringBuilder();
            String head = "<body class=\"cool\" style=\"padding:0;margin:0;text-align:center;background:#fbfbfb\">\n";
            ...
            for (int index = 0; index < refUrl.size(); index++) {
                String ref = refs.get(index);
                String elmt = constructElement(ref, refUrl.get(ref), refTheatre.get(ref));
                sb.append(elmt);
                if (index == 0) {
                    sb.append("      <ul class=\"xw3-ul \"" + " style=\"margin-top:0px;padding-top:0px\"" + ">\n  ");
                }
            }
            sb.append("</ul></ul>\n");
            return sb.toString();
    ...
    static class HtmlAreasLondon {
        Dirs dirs;
        String city;

        HtmlAreasLondon(Dirs dirs, String city) {
            this.dirs = dirs;
            this.city = city;
        }

        void execute() {
            List<String> areasDisplayLondon = London.areasDisplay();
            Collections.sort(areasDisplayLondon);
            int currentIndex = com.events.date.Calendar.indexOfCurrent();
            CalendarDate baseDate = com.events.date.Calendar.get(currentIndex + 1);
            for (int plusDays = 0; plusDays < 7; plusDays++) {
                CalendarDate cd = com.events.date.Calendar.get(currentIndex + 1 + plusDays);
                com.events.date.Calendar.Date date = cd.toDate();
                String weekday = WEEKDAYS_LONG.get(WEEKDAYS_ORDER.indexOf(cd.dayOfWeek));
                ...
                Util.write(path, html);
                if (plusDays == 0) {
                    Util.write(dirs.getHtmlDir() + "index.html", html);
                }
            }
        }
...
//            sb.append(menu);
            ...
            for (int i = 0; i < areas.size(); i++) {
                ...
                if (types.contains(area)) {
                    additionalStyle = "style=\"background:#fbfbfb\"";
                } else if ("anywhere".equals(area)) {
                    additionalStyle = "style=\"background:#fdf3f2;white-space:pre\"";
                }
        ...
        static String generateMenuNoLink(String marginbottom, String display) {
            display = displaySpaced(display);
        ...
        String generateUrl(String area, String datepart) {
            String areaurl = "anywhere".equalsIgnoreCase(area) ? "/guide/" + datepart : "/" + area + "/" + datepart;
            return Util.lowercase(areaurl);
        }

        String generateHeader() {
            return HtmlHeader.generate("Londonoo", FAVICON);
        }
    }

    static class HtmlHelper {
        static List<String> nextDays(String currentWeekday) {
            int i = WEEKDAYS_ORDER.indexOf(currentWeekday);
            List<String> weekdays = list();
            for (int j = 0; j < 31; j++) {
                String s = WEEKDAYS_ORDER.get((i + j) % 7);
                weekdays.add(uppercaseFirstLetter(s));
            }
            return weekdays;
        }

        static List<CalendarDate> nextDays(CalendarDate baseDate) {
            int i = com.events.date.Calendar.indexOf(baseDate);
            List<CalendarDate> toReturn = list();
            for (int j = 0; j < 31; j++) {
                toReturn.add(com.events.date.Calendar.get(j + i));
            }
            return toReturn;
        }

        static List<String> nextDaysLong(String currentWeekday) {
            int i = WEEKDAYS_ORDER.indexOf(currentWeekday);
            List<String> weekdays = list();
            for (int j = 0; j < 31; j++) {
                String s = WEEKDAYS_LONG.get((i + j) % 7);
                weekdays.add(uppercaseFirstLetter(s));
            }
            return weekdays;
        }
        ...
        static String hrefUnderline(String display, String link) {
            return "<a style=\"text-decoration:none;border-bottom:" + "1px solid #202020\" href=\"" + link.toLowerCase() + "\">" + display + "</a>";
        }
    }

    static class HtmlHeader {
        static String generate(String title, String favicon) {
            return "<!DOCTYPE html>\n" + "<html>\n" + "<title>" + title + "</title>\n" + generateMeta() + generateLinks(favicon) + generateStyle();
        }

        static String generateLinks(String favicon) {
            return "<link rel=\"icon\" href=\"" + favicon + "\"/>";
        }

        static String generateMeta() {
