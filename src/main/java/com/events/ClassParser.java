...
class ClassParser {
    static void make(Util.Url url, Context ctx, List<Element> elements, String redirectedUrl, Util.MultiList<Integer, Element> indexMap, Util.MultiList<Integer, Date> indexMy, Dirs dirs) {
        Multi3<List<String>, List<String>, Multi> exclusions = Parse.exclusions(ctx);
        sout("CLASS_METHODOLOGY running for " + size(elements) + " elements");
        for (Element element : elements) {
            Date date = elementParse(url, ctx, elements, redirectedUrl, indexMap, indexMy, exclusions, element, dirs);
            if (islistCut(ctx, date)) break;
        }
    }

    static boolean islistCut(Context ctx, Date date) {
        if (ctx.primaryListcut() && date != null && Calendar.CalendarDate.diffFromCurrent(date) > 90) {
            sout("LIST CUT reached 3 month point");
            return true;
        }
        return false;
    }
    ...
    static Date elementParse(Util.Url url, Context ctx, List<Element> elements, String redirectedUrl, Util.MultiList<Integer, Element> indexMap, Util.MultiList<Integer, Date> indexMy, Multi3<List<String>, List<String>, Multi> exc, Element element, Dirs dirs) {
        ...
        if (ctx.parseBlank() && ctx.parseTitleUrl()) {
            original = titleUrlLogic(link);
            text = original;
            if (text == null) {
                sout(link + "\nFILTERED ON TEXT NULL");
                return null;
            }
        }
        text = lowercase(text);
        ...
        if (!empty(indexMap) && !empty(indexMy)) {
            Integer elementIndex = indexMap.getaOne(element);
            ...
            if (empty(dates) && !ctx.parseBlank()) {
                sout("FILTERED ON DATES EMPTY");
                return null;
            }
        }
        Date fiveWeeksLater = Calendar.get(Calendar.indexOfCurrent() + 35).toDate();
        if (!Util.empty(dates)) {
            boolean earlyDateFound = false;
            for (Date date : dates) {
            ...
            if (!earlyDateFound) {
                sout("FILTERED ON LATE DATE");
                return null;
            }
        }
        if (ctx.parseTitleUrl()) {
            original = titleUrlLogic(link);
        ...
        if (secondLevelElements != null) {
            for (Element secondLevelElement : secondLevelElements) {
                secondLevel(url, ctx, redirectedUrl, exc, dates, secondLevelElements, secondLevelElement, dirs);
            }
            return null;
        } else {
            Event event = constructEvent(ctx, clean, link, dates, safeB(linkText), original, url.url, element, redirectedUrl, dirs);
    ...
    static List<Element> getSecondLevelElements(Context ctx, Element element) {
        List<Element> secondLevelElements = null;
        if (ctx.parseTimesplit()) {
            secondLevelElements = timesplitElements(ctx, element);
        } else if (ctx.parseCsub() != null) {
            secondLevelElements = element.getElementsByClass(ctx.parseCsub());
        }
    ...
    static void secondLevel(Util.Url url, Context ctx, String redirectedUrl, Multi3<List<String>, List<String>, Multi> exc, List<Date> dates, List<Element> secondLevelElements, Element secondLevelElement, Dirs dirs) {
        ...
        String textX = safeA(textHtmlX);
        if (empty(textX)) return;
        ...
        List<Date> csubdates;
        if (ctx.parseCsubx()) {
            csubdates = Parse.matchDates(ctx, cleanX);
            if (csubdates == null) return;
        } else {
            csubdates = Date.cloneMe(dates);
        }
    ...
    static boolean cexc(Context ctx, String text) {
        List<String> cexc = ctx.classExcText();
        if (!empty(cexc) && contains(text.toLowerCase(), lowercase(cexc))) {
            sout("FILTERED ON CEXC");
            return true;
        }
        return false;
    }

    static boolean creq2(Context ctx, String text) {
        List<String> creq2 = ctx.classReqText2();
        if (!empty(creq2) && !contains(text.toLowerCase(), lowercase(creq2))) {
            sout("FILTERED ON CREQ2");
            return true;
        }
        return false;
    }

    static boolean creq(Context ctx, String text) {
        List<String> creq = ctx.classReqtext();
        if (!empty(creq) && !contains(text.toLowerCase(), lowercase(creq))) {
            sout("FILTERED ON CREQ");
            return true;
        }
        return false;
    }

    static boolean reqclass(Context ctx, Element element) {
        String reqclass = ctx.classReqclass();
        if (!empty(reqclass) && !Jutil.hasClass(element, reqclass)) {
            sout("FILTERED ON REQCLASS");
            return true;
        }
        return false;
    }

    static boolean xwc(Context ctx, Element element) {
        String excWithClass = ctx.classExcWithClass();
        if (!empty(excWithClass) && Jutil.hasClass(element, excWithClass)) {
            sout("FILTERED ON XWC");
            return true;
        }
        return false;
    }

    static boolean creq0(Context ctx, String text) {
        List<String> creq0 = ctx.classReqtext0();
        if (!empty(creq0) && !contains(text.toLowerCase(), lowercase(creq0))) {
            sout("FILTERED ON CREQ0");
            return true;
        }
        return false;
    }

    static boolean pairExcluded(Context ctx, Element element) {
        Multi<String, String> excpair = ctx.classExcpair();
        if (!empty(excpair) && identifyPair(excpair, element)) {
            sout("FILTERED ON EXCPAIR");
            return true;
        }
        return false;
    }

    static boolean pairRequiredButNotFound(Context ctx, Element element) {
        Multi<String, String> reqpair = ctx.classReqpair();
        if (!empty(reqpair) && !identifyPair(reqpair, element)) {
            sout("FILTERED ON REQPAIR");
            return true;
        }
        return false;
    }
    ...
    static List<Element> timesplitElements(Context ctx, Element parent) {
        ...
        for (Element child : parent.children()) {
            ...
            if (time != null) timeSplitChildren.add(child);
        }
        return timeSplitChildren;
    }

    static boolean identifyPair(Multi<String, String> multi, Element element) {
        for (Element subclass : element.getElementsByClass(multi.a)) {
            String text = subclass.text();
    ...
    static Event constructEvent(Context ctx, StringMutable clean, String link, List<Date> dates, String linkText, String original, String source, Element element, String redirectedUrl, Dirs dirs) {
        ...
        boolean pass = Context.performLocreq(event, ctx);
        if (!pass) {
            return null;
        }
        if (ctx.parseAnchorId()) {
            String id = element.id();
            if (!empty(id)) link = source + "#" + id;
        }
        if (ctx.parseAnchorName()) {
            Elements linkElements = element.getElementsByTag("a");
            for (Element aTags : linkElements) {
                String name = aTags.attr("name");
                if (!empty(name)) {
                    link = source + "#" + name;
                    break;
                }
        ...
        } else if (!empty(ctx.link1Req()) && reqlink(ctx, link, ctx.link1Req(), dirs.getBaseDirs())) {
            sout("FILTERED ON REQLINK");
            return null;
        } else if (!empty(ctx.timeSkip()) && time == null) {
            Time timeSkiptime = TimeMatcher.matchSingle(lowercase(ctx.timeSkip()), false, ctx.lang, ctx.time);
        ...
        if (empty(event.dateTimes) && !empty(ctx.timeManual())) {
            ...
            for (Date date : dates) {
                event.dateTimes.put(date, timeManual);
            }
        }
        ...
        if (empty(dates) && empty(event.dateTimes)) {
            sout("FILTERED ON DATETIMES");
            return null;
        }
        if (!empty(event.dateTimes)) {
            for (Date date : event.dateTimes.keySet()) {
    ...
    static void parseNoTitle(Context ctx, String link, Event event) {
        if (ctx.parseNotitle()) {
            Multi3<Document, String, String> multi = WebReader.readParse(link);
            Document document = WebUtils.safeA(multi);
            if (document != null) {
                Elements titles = document.getElementsByTag("title");
                if (!empty(titles)) {
                    event.name = titles.get(0).text();
                }
            }
        }
    }
    ...
    static boolean almostBlank(Context ctx, StringMutable clean, String id) {
        if (Parse.check(clean.string) && !ctx.parseBlank()) {
            sout("FILTERED ON STRING NULL " + id);
            return true;
        }
        return false;
    }
    ...
    static boolean blank(Context ctx, StringMutable clean, String id) {
        if (empty(clean) && !ctx.parseBlank()) {
            sout("FILTERED ON CLEAN NULL " + id);
            return true;
        }
        return false;
    }

    static String titleUrlLogic(String link) {
        if (empty(link)) return null;
        String urlToWorkOn = link;
        if (link.endsWith("/")) {
            urlToWorkOn = substringRemoveLast(link);
        }
        urlToWorkOn = split(urlToWorkOn, "/").getLast();
        return urlToWorkOn.replaceAll("\\-", " ");
    }
