...
public class BetweenTwoTimes extends TimeMatcher {
    boolean pm_override;

    public BetweenTwoTimes(boolean pm_override) {
    ...
    protected List<Time> match(Util.StringMutable clean) {
        ...
        while (matcher.find()) {
            int start = "time".equals(matcher.group(1)) ? matcher.start() : matcher.start(3);
            int end = matcher.end();
            ...
            if (pm_override) {
                if (Util.empty(timeIndicative.amPm)) {
                    timeIndicative.amPm = "pm";
                }
            } else {
                if (!Util.empty(from_am_pm)) {
                    timeIndicative.amPm = from_am_pm;
                } else {
                    timeIndicative.amPm = intuitiveConversionA(Integer.valueOf(timeIndicative.getHour()), to_hour, Util.trim(matcher.group(11)));
                }
            }
            timeIndicative.convertTo24H();
            if (matcher.group(10).equals("9") && !(matcher.group(9).equals("5") && matcher.group(8).equals("23"))) {
                continue;
            }
...
}                   //59 allowed - its when pms not required
