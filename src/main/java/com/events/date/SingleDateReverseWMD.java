...
public class SingleDateReverseWMD extends DateMatcher {
    public DateMeta matchInternal(String text) {
        List<Calendar.Date> dates = list();
        text = cleanText(text);
        String weekday_month_day = "^" + M_WEEKDAY_ENG + "[,]?" + SPACES + M_MONTH_ENG + SPACES + M_DAY + SPACES;
        DateMeta date_internal = new DateMeta();
        Matcher matcher = Util.matcher(weekday_month_day, text);
        while (matcher.find()) {
            ...
            date.dateMonth = MONTHS_STANDARD_ENG.get(matcher.group(2));
