...
public class EventLink {
    public static final ArrayList<String> SECOND_CLICK_TEXT = list("book tickets", "book now", "buy tickets", "book", "view tickets");
    public static final String SECOND_CLICK_PARTIAL_TEXT = "check dates";
    public static final List<String> EXCLUDED_HOSTS = list("twitter.com");
    public static final List<String> LINK_TIME_AVOID_PHRASES_DIRECTIONAL = list("box office", "opening times", "opening hours");
    public static final List<String> LINK_TIME_AVOID_PHRASES_NON_DIRECTIONAL = list("tel:", "+44");
    ...
    static Util.Multi<Document, String> thirdLink(Context ctx, String link, Document doc, BaseDirs dirs) {
        ...
        if (ctx.link3Download()) {
            ...
            return new Util.Multi<>(safeA(docLocation), thirdLinkUrl);
        } else {
            Util.Multi3<Document, String, String> multi = WebReader.readParse(thirdLinkUrl);
            if (WebUtils.safeA(multi) != null) {
                return new Util.Multi<>(multi.a, thirdLinkUrl);
            }
        }
        return null;
    }

    static Util.Multi<Document, String> secondLink(Context ctx, String link, Document doc, BaseDirs dirs) {
        Map<String, String> hrefText = WebUtils.hrefText(doc);
        if (empty(hrefText)) return null;
        ...
        if (!empty(href2) && containsRev(href2, hrefText.keySet())) {
            hrefText.entrySet().removeIf(e -> !e.getKey().contains(href2));
        }
        String secondUrl = null;
        if (!empty(partialPermitted)) {
            for (String href : hrefText.keySet()) {
                String text = hrefText.get(href);
                if (safeNull(text).toLowerCase().startsWith(partialPermitted)) {
                    secondUrl = WebReader.UriExtension.toFullUrl(link, href);
                    break;
                }
            }
        }
        if (secondUrl == null && !empty(permittedSpecific)) {         //second href good -
            for (String href : hrefText.keySet()) {
                String text = hrefText.get(href);
                if (safeNull(text).toLowerCase().equals(permittedSpecific)) {
                    secondUrl = WebReader.UriExtension.toFullUrl(link, href);
                    break;
                }
            }
        ...
        if (secondUrl == null && !empty(permittedGeneral)) {
            for (String href : hrefText.keySet()) {
                String text = hrefText.get(href);
                if (permittedGeneral.contains(safeNull(text).toLowerCase())) {
                    secondUrl = WebReader.UriExtension.toFullUrl(link, href);
                    break;
                }
            }
        ...
        if (ctx.link2Download()) {
            ...
            if (WebUtils.safeA(multi) != null) {
                return new Util.Multi<>(multi.a, secondUrl);
            }
        }
        return null;
    }

    static Map<Calendar.Date, Time> linkTimesInternal(String link, Context ctx, List<Calendar.Date> dates, Document doc, BaseDirs dirs) {
        String pageText = cleanPageText(link, ctx, doc);
        if (!empty(ctx.linkxClass())) {
            return DateTimeParse.make(doc, ctx);
        }
        if (pageText == null) return null;
        if (ctx.parseBlank() && empty(dates)) {
            dates = DateMatcher.match(new Util.StringMutable(pageText), ctx.lang, ctx.date);
        }
        ...
        if (!empty(secondaryDateTimes)) {
            return secondaryDateTimes;
        } else {
            return dateTimes;
        }
    }

    static Map<Calendar.Date, Time> levelOneDateTimes(Context ctx, List<Calendar.Date> dates, Document doc) {
        ...
        if (!empty(linkclass)) {
            ...
            for (Element element : elements) {
                ...
                if (timeIndicative != null) {
                    for (Calendar.Date date : safeNull(dates)) {
                        dateTimes.put(date, timeIndicative.cloneMe());
                    }
                }
            }
            return dateTimes;
    ...
    static Map<Calendar.Date, Time> classicalPageTimes(Context ctx, String pageText, List<Calendar.Date> dates) {
        Map<Calendar.Date, Time> dateTimes = comboMatch(dates, pageText, ctx);
        if (!empty(dateTimes)) return dateTimes;
    ...
    static Map<Calendar.Date, Time> pageTime(Context ctx, List<Calendar.Date> dates, String pageText) {
        ...
        Time timeIndicative;
        if (empty(avoidIndicesDirectional) && empty(avoidIndicesNonDirectional)) {
            timeIndicative = TimeMatcher.matchOverall(clean, ctx.lang, ctx.time);
        } else {
            timeIndicative = TimeMatcher.matchAvoidIndices(clean, ctx.lang, ctx.time);
        }
        if (timeIndicative != null) {
            timeIndicative.provenance = "LINK_TIME_" + safeNull(timeIndicative.provenance);
    ...
    static String cleanPageText(String link, Context ctx, Document doc) {
        Util.Multi<String, Util.MultiList<Integer, Element>> page = Jutil.text(doc);
        String pageText = lowercase(safeA(page));
        if (empty(pageText)) return null;
        pageText = pageText.replaceAll("(\\s|\\u00A0)+", " ");
        if (pageText.length() > 100000 && !ctx.parseLong()) {
            sout("100000+ page length for " + ctx.ref + " " + link);
            return null;
        }
        return pageText;
    }
    ...
    static Map<Calendar.Date, Time> comboMatch(List<Calendar.Date> dates, String pageText, Context ctx) {
        ...
        if (!empty(rawDateTimes)) {
            if (ctx.parseBlank()) {
                for (Util.Multi<Calendar.Date, Time> multi : rawDateTimes.underlying) {
                    if (multi.a == null || multi.b == null) continue;
                    dateTimes.put(multi.a, multi.b);
                }
            } else {
                for (Calendar.Date date : dates) {
                    Time time = rawDateTimes.getBOne(date);
                    if (time != null) {
        ...
        for (Calendar.Date date : safeNull(dates)) {
            if (dateTimes.containsKey(date)) continue;
