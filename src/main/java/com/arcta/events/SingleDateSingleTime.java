...
class SingleDateSingleTime extends DateTimeMatcher {boolean pm; boolean doors;
    SingleDateSingleTime(boolean pm, boolean doors){this.pm = pm;this.doors = doors;}
    Util.MultiList<Calendar.Date, Time> matchInternal(String text) { Util.MultiList<Calendar.Date, Time> dateTimes = new Util.MultiList<>();
        ...
        while (matcher.find()) {
            ...
            if (doors && !Util.empty(matcher.group(11))) {hourInt = hourInt + 1;}
            ...
            if (pm) time.amPm = "pm";
            time.convertTo24H();
            dateTimes.add(new Util.Multi<>(date, time));} return dateTimes;}}       //(sat) 3(rd) jan (sat) (2020) 7:30(pm)