...
class Link {
    ...
    static Util.Multi<String,String> make(String url, Collection<Element> parentElements, Element classElement, Context ctx) {
        String badHrefSpecific = ctx.linkBadHref(); String poorHrefSpecific = ctx.linkPoorHref();
        String goodHrefSpecific = ctx.linkGoodHref(); String goodTextSpecific = ctx.linkGoodText();
        ...
        if (!Util.empty(hrefText)) {
            ...
            for (String href : hrefText.keySet()) {            // -------------- GOOD ----------------
                ...
                if (!Util.empty(goodHrefSpecific)) {
                    if (href.contains(goodHrefSpecific)) {return new Util.Multi<>(WebReader.UriExtension.toFullUrl(url, href), text);}}
                if (Util.empty(text)) continue;
                if (Util.contains(text.toLowerCase(), GOOD_TEXT)) {return new Util.Multi<>(WebReader.UriExtension.toFullUrl(url, href), text);}
                if (Util.contains(text.toLowerCase(), goodTextSpecific)) {return new Util.Multi<>(WebReader.UriExtension.toFullUrl(url, href), text);}}
            ...
            for (String href : hrefText.keySet()) {String text = hrefText.get(href);
                ...
                if (Util.startsWith(text.toLowerCase(), POOR_TEXT_BEGINNINGS)) {lowPriorityHrefs.add(href);
                } else if (Util.contains(text.toLowerCase(), POOR_TEXT)) {lowPriorityHrefs.add(href);
                } else if (POOR_TEXT_EQUALS.contains(text.toLowerCase())) {lowPriorityHrefs.add(href);
                } else if (ctx.ref.equals(text.toLowerCase())) {lowPriorityHrefs.add(href);
                } else if (Util.contains(href.toLowerCase(), POOR_HREF)) {lowPriorityHrefs.add(href);
                } else if (Util.contains(href.toLowerCase(), poorHrefSpecific)) {lowPriorityHrefs.add(href);
                } else if (Util.safeNull(text).length() < 15 && Util.safeNull(text).length() > 4) {Util.StringMutable clean = new Util.StringMutable(Util.lowercase(text));
                    DateMatcher.match(clean, ctx.lang, ctx.date);
                    if (Util.empty(clean.string)) {lowPriorityHrefs.add(href);}}}
            ...
            for (String href : lowPriorityHrefs) {             // --------------- NOT BAD --------------------
                ...
                if (href.equals("#") || href.equals(url + "/#") || href.equals(url + "#")) continue;
                return new Util.Multi<>(WebReader.UriExtension.toFullUrl(url, href), hrefText.get(href));}}
        if (parentElements == null) return null;         // -----LOOK FOR INDICATIVE URL IN PARENT SO LONG AS THE PARENT DOES NOT ALSO HAVE OTHER CHILDREN WITH // ----------------------------------THE SAME CLASS NAME -----------------------------------------
