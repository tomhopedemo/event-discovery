...
class BetweenTwoTimesAmPm extends TimeMatcher {
    List<Time> match(Util.StringMutable clean) {String text = clean.string;
        ...
        while (matcher.find()) {
            ...
            } else { timeIndicative.amPm = intuitiveConversionA(Integer.valueOf(timeIndicative.getHour()), to_hour, matcher.group(9).trim());}
            ...
            to_return.add(timeIndicative);} return to_return;}}           //am/pm required only at the end - e.g. 3-3.30pm.
