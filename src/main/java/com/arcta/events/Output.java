...
class Output {
    ...
    static Map<Util.Multi<String, Calendar.Date>, String> filteredByDate;
    ...
    static void execute(List<String> refs, Dirs dirs, InputData inputData) {
        ...
        for (Event event : events) {
            ...
            if (!Util.empty(rday)) {
                event.dateTimes.entrySet().forEach(e -> Calendar.setDayOfWeek(e.getKey()));
            ...
            } else if (totalDates > highlyDistributedLimit && (Util.safeNull(Calendar.Date.daysBetween(event.dateTimes.keySet())) > 25) && !Theatre.isTheatre(event)) {
    ...
    static void removeRepetitions(Util.MultiList<Multi<Calendar.Date, Time>, Event> dateTimeEvents) {
        ...
        for (Util.Multi<Calendar.Date, Time> dateTime : map.keySet()) {
    ...
    static void removeCrossReferences(Util.MultiList<Multi<Calendar.Date, Time>, Event> dateTimeEvents) {
        Map<Util.Multi<Calendar.Date, Time>, ArrayList<Event>> map = dateTimeEvents.map();
        for (Util.Multi<Calendar.Date, Time> dateTime : map.keySet()) {
            ArrayList<Event> events = map.get(dateTime);
            external:
    ...
    static void cleanDoubleDateSpecialDates(Event event) {
        ...
        for (Map.Entry<Calendar.Date, Time> e : event.dateTimes.entrySet()) {
            Calendar.Date date = e.getKey();
    ...
    static String generateUrl(Util.Multi<Util.Multi<Calendar.Date, Time>, Util.Multi<Event, Boolean>> multi) {
        String url;
    ...
    static boolean exclusions1(Event event) {
        List<Calendar.Date> dates = Util.sort(event.dateTimes.keySet());
        if (Util.empty(dates)) {
    ...
    static class DateTimeComparator implements Comparator<Object> {
        public int compare(java.lang.Object o1, java.lang.Object o2) {
            ...
            Util.Multi<Util.Multi<Calendar.Date, Time>, Event> multi1 = (Util.Multi) o1;
            Util.Multi<Util.Multi<Calendar.Date, Time>, Event> multi2 = (Util.Multi) o2;
