...
class TableParser {
    static void make(String url, Context ctx, List<Element> elements, Util.MultiList<Integer, Element> indexElements, Util.MultiList<Integer, Calendar.Date> indexMy, BaseDirs dirs) { boolean foundTable = false;
        Map<Element, String> elementText = map();
        for (Element element : elements) {
            if ("table".equals(element.tagName())) { foundTable = true;
                elementText.put(element, element.text());}}
        if (foundTable) { tableMethodology(url, ctx, elementText, indexElements, indexMy, dirs);
        } else {
            for (Element element : elements) { Element table = Jutil.childElementByTag(element, "table");
                if (table != null) { foundTable = true;
    ...
    static void tableMethodology(String url, Context ctx, Map<Element,String> elementText, Util.MultiList<Integer, Element> indexElements, Util.MultiList<Integer, Calendar.Date> indexMy, BaseDirs dirs) {
        for (Element element : elementText.keySet()) { Element tbody = Jutil.childElementByTag(element,"tbody");
            ...
            for (Element rowElement : Jutil.childElementsByTag(tbody,"tr")) {
                ...
                if (Util.empty(dates)) {
                    if (!Util.empty(indexElements) && !Util.empty(indexMy)) { Calendar.Date dateMy = Util.interpolateBefore(indexMy, indexElements.getaOne(rowElement));
                ...
                event.source = url;
                if (time == null && link != null && !ctx.parseNolink()) {event.dateTimes = EventLink.make(link, ctx, dates, dirs);
                } else if (time != null) {dates.forEach(d -> event.dateTimes.put(d, time));
                } else {sout("FILTERED ON TABLE DATETIMES"); continue;}
                if (!Util.empty(event.dateTimes)) {
                    for (Calendar.Date date : event.dateTimes.keySet()) {event.dateUrl.put(date, event.link);}}
