...
import static com.arcta.events.DateMatcher.constructIntermediateDates;
import static com.arcta.events.M_Static.*;
import static com.arcta.events.Util.*;
class Page {
    static List<Url> generateUrls(Context ctx, InputData inputData) { List<Url> urls = urlConversion(ctx, inputData);
        urls.removeIf(e -> (e == null || empty(e.url)));
        return urls;}
    static List<Url> urlConversion(Context ctx, InputData inputData) { Set<Url> toReturn = set();
        List<String> urls = inputData.getUrls(ctx.ref); if (empty(urls)) return list();
        for (String url : urls) { Set<Url> dateModifiedUrls = set(); List<Url> paginatedUrls = list();
            Calendar.CalendarDate from = Calendar.currentCalendarDate();
            int fromIndex = Calendar.indexOf(from);
            List<Calendar.Date> dates; Calendar.CalendarDate until;
            if (ctx.pageWeekly()){ until = Calendar.get(fromIndex + 40);
                dates = constructIntermediateDates(from.toDate(), until.toDate(), true);
                dates.removeIf(date -> !date.dayOfWeek.equals(from.dayOfWeek));
            }  else if (ctx.pageMonthly() || Util.contains(url, list("ZZZZ-NN-DD", "VVVVVVVVVV"))){ until = Calendar.get(fromIndex + 40);
                dates = constructIntermediateDates(from.toDate(), until.toDate(), true);
                dates.removeIf(d -> !(d.equals(from.toDate()) || d.dateDay.equals("1")));
            } else { until = Calendar.get(fromIndex + 20);
                dates = constructIntermediateDates(from.toDate(), until.toDate(), true);}
            ...
            for (String template : pageTemplates) {if (url.contains(template)) pageTemplate = template;}
            if (url.contains("YYYYMMDD-YYYYMMDD")){ String daypadleft = Util.padleft(from.dateDay, '0', 2);
            ...
            } else if (url.contains("m=N")) { String newUrl = url.replace("YYYY", from.dateYear).replace("ZZZZ", until.dateYear).replace("m=M", "m=" + (MONTHS_ORDER.indexOf(from.dateMonth) + 1)).replace("m=N", "m=" + (MONTHS_ORDER.indexOf(until.dateMonth) + 1)).replace("d=D", "d=" + from.dateDay).replace("d=E", "d=" + until.dateDay);
                dateModifiedUrls.add(new Url(newUrl));
            } else if (containsDateTemplate) {
                for (Calendar.Date date : dates) { Calendar.CalendarDate plusOneMonth = Calendar.get(Calendar.indexOf(date)  + 31);
            ...
            } else { dateModifiedUrls.add(new Url(url));}
            int pNum = 5; int qNum = 5; int rNum = 5; int lNum = 3;
            if (pageTemplate != null) {
                for (Url modifiedUrl : dateModifiedUrls) { List<String> pageTemplates1 = list("page=P","Page=P","pg=P","page=Q","page-P","paged=P","page/P","paged=Q","page/L");
                    if (Util.contains(modifiedUrl.url, pageTemplates1)) { paginatedUrls.add(new Url(modifiedUrl.url.replaceAll(pageTemplate, "")));
                        String delimiter = "=";
                        ...
                        if (containsSingleDateTemplate){
                            for (int i = 1; i < 10; i++) { paginatedUrls.add(new Url(modifiedUrl.url.replaceAll(pageTemplate, Util.split(pageTemplate,delimiterRegex).get(0) + delimiter + i)));}
                        } else if (containsDateTemplate) { paginatedUrls.add(new Url(modifiedUrl.url.replaceAll(pageTemplate, Util.split(pageTemplate,delimiterRegex).get(0) + delimiter + "1")));
                            ...
                            if (Util.contains(modifiedUrl.url, list("page=Q","paged=Q"))){ num = qNum;
                            } else if (modifiedUrl.url.contains("page/L")){ num = lNum;
                            } else { num = pNum;}
                            for (int i = 0; i < num; i++) { String newUrl = modifiedUrl.url.replaceAll(pageTemplate, Util.split(pageTemplate, delimiterRegex).get(0) + delimiter + i);
                                paginatedUrls.add(new Url(newUrl));}}
                    } else if (modifiedUrl.url.contains("rank=RR")) {
                        for (int i = 0; i < rNum; i++) {paginatedUrls.add(new Url(modifiedUrl.url.replaceAll("rank=RR", "rank=" + (1 + (i*10)))));}
                    } else { paginatedUrls.add(modifiedUrl);}}}
            if (!empty(paginatedUrls)){ toReturn.addAll(paginatedUrls);
            } else if (!empty(dateModifiedUrls)){ toReturn.addAll(dateModifiedUrls);
            } else { toReturn.add(new Url(url));}} return new ArrayList<>(toReturn);}}