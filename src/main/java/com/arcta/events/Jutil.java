...
class Jutil {
    static Element childElementByTag(Element parent, String tag){ if (parent == null || Util.empty(tag)) return null;
        List<Element> elements = childElementsByTag(parent, tag);
        if (Util.empty(elements)) return null;
        return elements.get(0);}
    static List<Element> childElementsByTag(Element parent, String tag) { if (tag == null || parent == null) return null;
        List<Element> childrenTag = list();
        Elements children = parent.children();
        for (Element child : children) {
            if (tag.equals(child.tagName())) childrenTag.add(child);} return childrenTag;}
    static boolean hasClass(Element element, String className){ List<String> list = classesRecursive(element.children(), list());
    ...
    static List<String> classesRecursive(Elements elements, List<String> foundClasses) {
        ...
        for (Element element : elements) { if (element == null) continue;
            Set<String> classNames = element.classNames();
            if (!Util.empty(classNames)) {foundClasses.addAll(classNames);}
            classesRecursive(element.children(), foundClasses);}
        return foundClasses;}
    static List<Element> getElementsByClassFallback(Element doc, String identifier){ List<Element> elements = doc.getElementsByClass(identifier);
        if (!Util.empty(elements)) return elements;
        if (Util.empty(identifier)) return elements;
        if (identifier.contains("-")) { elements = doc.getElementsByClass(identifier.replaceAll("\\-", "_"));
        } else if (identifier.contains("_")) { elements = doc.getElementsByClass(identifier.replaceAll("_", "-"));}
        return elements;}
    ...
    static void indexElementByTagRecursive(List<Element> elements, String tagIdentifier, Util.MultiList<Integer, Element> indexElements, Util.IntegerMutable currentIndex){
        for (Element element : elements) {
            if (tagIdentifier.equals(element.tagName())){ indexElements.add(new Util.Multi(currentIndex.integer, element));}
    ...
    static Util.MultiList<Integer,Element> indexElement(Element document, String classIdentifier){ Elements children = document.children();
        Util.MultiList<Integer, Element> indexElements = new Util.MultiList<>();
    ...
    static void indexElementRecursive(List<Element> elements, String classIdentifier, Util.MultiList<Integer, Element> indexElements, Util.IntegerMutable currentIndex){
        for (Element element : elements) {
            if (element.classNames().contains(classIdentifier)){ indexElements.add(new Util.Multi(currentIndex.integer, element));}
            currentIndex.integer = currentIndex.integer + 1;
    ...
    static Util.Multi<String, Util.MultiList<Integer, Element>> text(List<Node> elements, List<String> excludedTags, List<String> excludedClasses, Util.Multi<String,String> excludedAttributes,  List<String> restrictedToTags, List<String> restrictedToClasses, List<String> restrictedToIds, Util.Multi<String,String> restrictedToAttribute, String gapClass, String gapTag) {
        ...
        for (Util.Multi<String,Element> stringElement : stringElements.underlying) {
            ...
            if (string.length() == 0 && stringElement.b != null){ indexElement.add(new Util.Multi<>(sb.length(), stringElement.b));
            } else { indexElement.add(new Util.Multi<>(sb.length(), stringElement.b));
                sb.append(string);
                sb.append(" "); }}
    ...
    static Util.Multi<String, Util.MultiList<Integer, Element>> text(Element element, List<String> excludedTags, List<String> excludedClasses, Util.Multi<String,String> exclusionAttribute, List<String> restrictedToTags, List<String> restrictedToClasses, List<String> restrictedToIds, Util.Multi<String,String> restrictedToAttribute, String gapClass, String gapTag){
        ...
        if (childNodes == null || childNodes.size() == 0){  return new Util.Multi(element.text(), Util.map(0, element)); }
    ...
    static Util.MultiList<String, Element> textFinderRecursive(List<Node> elements, Util.MultiList<String,Element> list, List<String> excludedTags, List<String> excludedClasses,Util.Multi<String, String> excludeAttribute, List<String> restrictedToTags, List<String> restrictedToClasses, List<String> restrictedToIds,Util.Multi<String,String> restrictedToAttribute, String gapClass, String gapTag) {
        for (Node node: elements) {
            if (node instanceof Element) { Element element = (Element) node; //Indicate the start of a Header Element
                if (HEADING_TAGS.contains(element.tagName())){list.add(new Util.Multi<>("\u2021", element));}
                if (element.tagName().equals("a") && !Util.empty(element.attributes().get("href"))) {list.add(new Util.Multi<>("", element));}
                ...
                if (element.childNodes() == null || element.childNodes().size() == 0) { String ownText = element.ownText();
                ...
                } else { textFinderRecursive(element.childNodes(), list, excludedTags, excludedClasses, excludeAttribute, restrictedToTags, restrictedToClasses, restrictedToIds, restrictedToAttribute, gapClass, gapTag);} //Indicate the end of a Header Element - insert additional text
                if (HEADING_TAGS.contains(element.tagName()) || Util.intersection(Util.lowercase(element.classNames()), TITLE_CLASSES)) { list.add(new Util.Multi<>("\u2021", element));
                } else if (Util.endswithReverse(TITLE_CLASS_ENDING, Util.lowercase(element.classNames()))){ list.add(new Util.Multi<>("\u2021", element)); //indicate the end of another type of major grouping element -
                } else if (MAJOR_GROUPINGS.contains(element.tagName())){ list.add(new Util.Multi<>("\u2021",null));
                } else if (gapClass != null && Util.safeNull(element.classNames()).contains(gapClass)){ list.add(new Util.Multi<>("\u2021", null));
                } else if (gapTag != null && Util.safeNull(element.tagName()).equals(gapTag)){ list.add(new Util.Multi<>("\u2021", null));} //indicate end of a href
                if (element.tagName().equals("a") && !Util.empty(element.attributes().get("href"))) { list.add(new Util.Multi<>("", element));}
    ...
    static Set<String> allParentClassesNode(Node node) {
        ...
        for (Element parent : parentElement.parents()) {Util.addAll(classNames, parent.classNames());}
        return classNames;}
    static Set<String> allParentTagsNode(Node node) { Set<String> tagNames = set();
        Element parentElement = (Element) node.parent();
        Util.addIfNotEmpty(tagNames, parentElement.tagName());
        for (Element parent : parentElement.parents()) {Util.addIfNotEmpty(tagNames, parent.tagName());} return tagNames;}
    static Set<String> allParentClasses(Element element) { Set<String> classNames = set();
        Util.addAll(classNames, element.classNames());
        for (Element parent : element.parents()) {Util.addAll(classNames, parent.classNames());}
        return classNames;}
    static Set<String> allParentIdsNode(Node node) { Set<String> idNames = set();
        Element parentElement = (Element) node.parent();
        Util.addIfNotEmpty(idNames, parentElement.id());
        for (Element parent : parentElement.parents()) {Util.addIfNotEmpty(idNames, parent.id());}
        return idNames;}
    static Set<String> allParentIds(Element element) { Set<String> idNames = set();
        Util.addIfNotEmpty(idNames, element.id());
        for (Element parent : element.parents()) {Util.addIfNotEmpty(idNames, parent.id());} return idNames;}
    static Set<String> allParentTags(Element element) { Set<String> tagNames = set();
        Util.addIfNotEmpty(tagNames, element.tagName());
        for (Element parent : element.parents()) {Util.addIfNotEmpty(tagNames, parent.tagName());} return tagNames;}
    static Set<String> allParentAttr(Element element, String attr) { Set<String> attrValues = set();
        Util.addIfNotEmpty(attrValues, element.attr(attr));
        for (Element parent : element.parents()) {Util.addIfNotEmpty(attrValues, parent.attr(attr));} return attrValues;}
    static String textOnly(Element element) { if (element == null) return null;
        Util.Multi<String, Util.MultiList<Integer, Element>> text = text(element); return Util.safeA(text);}
    static Util.Multi<String, Util.MultiList<Integer, Element>> text(Element element) {return text(element, null, null, null, null, null, null, null, null, null);}}