...
class Util {
    static final List<String> HYPHENS = Util.list("-", "\u2010", "\u2013", "\u2014", "\u002d");

    static String substring(int startInclusive, int endExclusive, String input) {
    ...
    static <T> T getFirst(Set<T> set) {
        if (empty(set)) return null;
        return set.iterator().next();
    }

    static <T extends Comparable> List<T> sort(Collection<T> collection) {
    ...
    static List<Integer> integerList(int startInclusive, int endInclusive) {
    ...
    static Integer proximalIndex(String whole, int index, String searchTerm, int radius) {
    ...
    static String removeBetween(int startInclusive, int endExclusive, String input) {
    ...
    static String removeLast(String toRemove, String input) {
        if (Util.empty(input)) return input;
    ...
    static String uppercaseFirstLetter(String input, Map<String, String> lowercaseUppercaseCharmap) {
        if (Util.empty(input)) return input;
    ...
    static boolean endsWith(String input, List<String> endings) {
    ...
    static int indexOf(String string, List<String> toFind) {
    ...
    static <T> Set<T> intersect(Collection<T> a, Collection<T> b) {
        Set<T> intersect = set();
    ...
    static boolean empty(MultiList multiList) {
    ...
    static boolean empty(Multi multi) {
    ...
    static boolean empty(MapList mapList) {
        return (mapList == null || mapList.mapList == null || empty(mapList.mapList));
    }

    static boolean empty(StringMutable stringMutable) {
    ...
    static void addAll(MultiList existing, MultiList newElements) {
        if (existing == null || newElements == null) return;
        addAll(existing.underlying, newElements.underlying, null);
    }

    static <T> void addAll(Collection<T> existing, Collection<T> newElements, Integer limit) {
        if (existing == null || newElements == null) return;
        if (limit != null) {
    ...
    static <T> List<T> sublist(List<T> list, int num) {
        if (list == null) return null;
        return list.subList(0, Math.min(num, list.size()));
    }

    static <S, T> MultiList<S, T> sublist(MultiList<S, T> list, int num) {
    ...
    static <T> T interpolateBefore(MultiList<Integer, T> multiList, Integer key) {
    ...
    static List<Integer> indices(String text, List<String> toFind) {
    ...
    static <E> HashSet<E> set(E... objects) {
    ...
    static boolean empty(NList collection) {
        return (collection == null || collection.size() == 0 || (collection.size() == 1 && collection.underlying.iterator().next() == null));
    }

    static <T> void safeAdd(Collection<T> existing, Collection<T> newItems) {
        if (existing == null || newItems == null) return;
        existing.addAll(newItems);
    }

    static Integer minDifferenceDirectional(List<Integer> collection, int value) {
    ...
    static Integer min(Collection<Integer> collection) {
        if (empty(collection)) return null;
        Integer min = null;
    ...
    static Integer minDifference(List<Integer> collection, int value) {
        if (empty(collection)) return null;
        List<Integer> distancesBetween = list();
    ...
    static <A, B> void addSafe(Map<A, B> existing, Map<A, B> toAdd) {
        if (existing == null) return;
        if (empty(toAdd)) return;
        existing.putAll(toAdd);
    }

    static <T, S> HashMap<T, S> map() {
        return new HashMap<>();
    }

    static <T, S> HashMap<T, S> map(T key, S value) {
    ...
    static String safeNull(String value) {
        return value == null ? "" : value;
    }

    static <T, S> T safeA(Multi<T, S> multi) {
        if (multi == null) return null;
        return multi.a;
    }

    static <T, S> S safeB(Multi<T, S> multi) {
        if (multi == null) return null;
        return multi.b;
    }

    static NList splitSafe(String string, String delimiterRegex) {
        if (string == null) {
    ...
    static String padleft(String input, Character pad, Integer maxChars) {
    ...
    static <S extends Comparable<S>, T> void sortA(MultiList<S, T> multiList) {
    ...
    static <S, T> MultiList multilist(Multi<S, T> m) {
    ...
    static String removeIndices(String string, Collection<Integer> indicesToRemove) {
    ...
    static String stringChar(List<Character> characterArray) {
    ...
    static List<Integer> between(int startInclusive, int endInclusive) {
        List<Integer> toReturn = list();
        for (int i = startInclusive; i <= endInclusive; i++) {
            toReturn.add(i);
        }
    ...
    static <T> List<T> safeNull(List<T> a) {
        return empty(a) ? new ArrayList<T>() : a;
    }

    static <T> Set<T> safeNull(Set<T> a) {
    ...
    static String substringRemoveLast(StringBuilder sb) {
        if (sb.length() == 0) {
            return "";
        } else {
            return sb.substring(0, sb.length() - 1);
    ...
    static String startsWithGet(String input, Collection<String> inputs) {
    ...
    static String substringRemoveFirst(String sb) {
    ...
    static <T> void sout(List<T> list, String pad) {
    ...
    static Set<String> keysForValues(Set<String> values, Map<String, String> map) {
        Set<String> keys = set();
    ...
    static <T> String string(T[] collection) {
        if (collection == null) return null;
    ...
    static boolean between(int middle, int aInclusive, int bInclusive) {
        return middle >= Math.min(aInclusive, bInclusive) && middle <= Math.max(aInclusive, bInclusive);
    }

    static List<String> lowercase(Collection<String> collection) {
        if (collection == null) return null;
    ...
    static <T> T getElement(List<T> set, T object) {
        return get(set, set.indexOf(object));
    }

    static Matcher matcher(String regex, String text) {
        return Pattern.compile(regex).matcher(text);
    }

    static String getIgnoreCase(Collection<String> collection, String word) {
        for (String s : collection) {
    ...
    static <A, B, C> A safeA(Multi3<A, B, C> multi) {
        if (multi == null) return null;
        return multi.a;
    }

    static <A, B, C> B safeB(Multi3<A, B, C> multi) {
        if (multi == null) return null;
        return multi.b;
    }

    static boolean endswithReverse(String ending, List<String> strings) {
    ...
    static <T, S> void putIfAbsent(Map<T, S> existing, Map<T, S> additionalMap) {
    ...
    static boolean complete(Multi multi) {
    ...
    static boolean containsRev(String input, Collection<String> inputs) {
    ...
    static Integer safeNull(Integer value) {
        return value == null ? 0 : value;
    }

    static <T> List<T> except(Collection<T> collection, Collection<T> exceptions) {
        if (collection == null) return null;
    ...
    static <T> String string(Collection<T> collection) {
        if (collection == null) return null;
        return out(collection, " ");
    }

    static <S, T> List<T> bList(MultiList<S, T> multiList) {
        if (empty(multiList)) return list();
        return multiList.getBList();
    }

    static String containsGet(String input, Collection<String> inputs) {
        if (inputs == null || input == null || input.equals("")) return null;
    ...
    static <T extends Collection<String>> T addIfNotEmpty(T collection, String string) {
    ...
    static double roundedToDisplayPerDay(double numPerDay) {
    ...
    static String cleanInt(String raw) {
        return valueOf(parseInt(raw.trim()));
    }

    static Double divide(Integer numerator, Integer denominator) {
    ...
    static String opt(String input) {
    ...
    static final List<String> FILE_LOCKS_REQUESTED_BY_READER = list();
    static final List<String> FILE_LOCKS_REQUESTED_BY_WRITER = list();
    ...
    static void appendSafe(String path, String valueToWrite) {
        synchronized (path) {
            try {
    ...
    static void append(String path, String valueToWrite, String delimiter) {
        synchronized (path) {
    ...
    static void append(String path, String valueToWrite) {
        append(path, valueToWrite, "\n");
    }
    ...
    static void appendInternal(String path, String valueToWrite) {
        PrintWriter out = null;
        try {
            File file = new File(path);
            if (file.getParentFile() != null) {
    ...
    static Map<String, String> readMap(String file, String delimiter) {
    ...
    static List<List<String>> readTable(String file, String delimiter) {
    ...
    static class Url {
        ...
        Url(String url, Calendar.Date date) {
            this.url = url;
            this.date = date;
        }
    ...
    static class StringMutable {
        String string;

        StringMutable(String string) {
            this.string = string;
        }
    ...
    static class Table {
        List<List<String>> underlying;

        Table(List<List<String>> underlying) {
            this.underlying = underlying;
        }

        MultiList<Integer, Integer> getIgnoreCase(String value) {
        ...
        Map<String, String> createMap(int columnA, int columnB) {
            Map<String, String> map = map();
        ...
        List<String> createList(int column) {
            List<String> list = list();
            for (List<String> row : underlying) {
    ...
    static class NList {
        ...
        String reconstruct(List<String> list, String joiner) {
            if (list == null) return null;
        ...
        String reconstruct(int index_from_inclusive, int index_to_exclusive) {
            if (underlying == null) return null;
            List<String> sublist = sublist(index_from_inclusive, index_to_exclusive);
            return reconstruct(sublist);
        }

        String reconstruct(int index_to_exclusive) {
            if (underlying == null) return null;
            List<String> sublist = sublist(0, index_to_exclusive);
            return reconstruct(sublist);
        }

        String reconstruct(int index_to_exclusive, String joiner) {
            if (underlying == null) return null;
            List<String> sublist = sublist(0, index_to_exclusive);
        ...
        String reconstructReplacing(Integer index, List<String> strings) {
        ...
        String reconstructReplacing(Map<Integer, String> index_replacement) {
            List<String> words = list();
            for (int i = 0; i < underlying.size(); i++) {
        ...
        String reconstructReplacing(Integer index, String replacement) {
            return reconstructReplacing(map(index, replacement));
        }

        String reconstructExcept(List<Integer> except) {
            if (underlying == null) return null;
        ...
        List<String> sublist(int startIndex, int endIndexExclusive) {
    ...
    static class Film {
        static final List<String> filmRatings = list("(U)", "(PG)", "(12)", "(15)", "(18)", "(12A)", "[U]", "[PG]", "[12]", "[15]", "[18]", "[12A]", "P&B");
    }

    static class IntegerMutable {
        Integer integer = 0;
    }
    ...
    static class Months {
        static final Map<String, String> MONTHS_STANDARD_ENG;
        static final Map<String, String> MONTHS_STANDARD_DEU;
    ...
    static class Multi<A, B> implements Serializable {
        A a;
        B b;
    ...
    static class MapList<K extends Comparable<? super K>, T> implements Serializable {
        ...
        void addAll(K key, Collection<T> elements) {
            mapList.putIfAbsent(key, list());
    ...
    static class Multi3<A, B, C> implements Serializable {
        A a;
        B b;
        C c;
    ...
    static class MultiList<S, T> {
        List<Multi<S, T>> underlying = list();

        MultiList() {
        }

        void add(Multi<S, T> multi) {
        ...
        <U extends Multi<S, T>> void addAll(Collection<U> multis) {
            underlying.addAll(multis);
        }

        Map<S, ArrayList<T>> map() {
