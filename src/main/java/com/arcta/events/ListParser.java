...
class ListParser {
    static void make(String url, Context ctx, List<Element> elements, String redirectedUrl, BaseDirs dirs) { Util.Multi3<List<String>, List<String>, Util.Multi> exc = Parse.exclusions(ctx);
        for (Element element : elements) { if (element == null) continue;
            ...
            if (Util.empty(overallDates)) {monthYear = Util.get(DateMatcher.matchMonthYear(subsentence),0);}
            List<Element> lis = Jutil.childElementsByTag(supertagElement, "li");
            if (lis == null) continue;
            for (Element listElement : lis) { Util.Multi<String, Util.MultiList<Integer, Element>> textHtml = Jutil.text(listElement, exc.b, exc.a, exc.c, null, null, null, null, ctx.parseGap(), ctx.parseTagGap());
                ...
                } else { dates.forEach(d -> event.dateTimes.put(d, time));}
