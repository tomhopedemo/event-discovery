...
class Replace {
    static String replace(String title) {
        ...
        String mPunctuation = "(\u2010|\u2013|\u2014|\\-|;|:|\\.|\\|)";
        for (int i = 0; i < 3; i++) { title = title.replaceAll(mPunctuation + "\\s*" + mPunctuation, " - ");
            title = Util.trim(title);} return title;}
    static String properHyphenation(String title) { Util.NList split = Util.split(title, " ");
        for (int i = 0; i < split.size(); i++) { String word = split.get(i);
            if (word.length() > 1 && word.endsWith("-")){ List<String> words = Util.list(word.substring(0, word.length() -1), "-");
                title = split.reconstructReplacing(i, words);
                break;}} return title;}
    static void cleanPunctuation(Util.StringMutable clean){ if (Util.empty(clean)) return;
        String string = clean.string;
        if (string.startsWith(",")){ string = string.replaceAll("^,\\s+","");}
