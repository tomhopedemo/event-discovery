...
class WebReader { static final String DELIMITER = "#ARCTA#";
    interface BooleanMutable {boolean check();}
    ...
    static void setCaches(String root, String slash){
        WEBCACHE = root; SLASH = slash;
        PHANTOM_TEMP = WEBCACHE + "phantomtemp" + slash + "output.txt"; GOOGLE_TEMP = WEBCACHE + "googletemp" + slash + "output.txt";
        CACHE_FOLDER = WEBCACHE + "cached" + slash;
        CACHE_FOLDER_PHANTOM = WEBCACHE + "cached_phantom"+ slash; CACHE_FOLDER_GOOGLE = WEBCACHE + "cached_google"+ slash;
        CACHE_FOLDER_CURL = WEBCACHE + "cached" + slash + "curl" + slash;
        CACHE_FOLDER_HEADLESS = WEBCACHE + "cached" + slash + "headless" + slash; MAPPING_FILE_CURL = CACHE_FOLDER + "mapping_curl.txt";
        MAPPING_FILE_SIMPLE_READ = CACHE_FOLDER + "simple_mapping.txt"; MAPPING_FILE_HEADLESS = CACHE_FOLDER + "mapping_headless.txt";
        REDIRECTION_FILE_CURL = CACHE_FOLDER + "redirection_curl.txt"; REDIRECTION_FILE_SIMPLE_READ = CACHE_FOLDER + "redirection_simple.txt";
        REDIRECTION_FILE_HEADLESS = CACHE_FOLDER + "redirection_headless.txt"; TEXT_MAPPING_FILE = CACHE_FOLDER + "text_mapping.txt";
        loadCaches();}
    ...
    static class Advanced {
        static Util.Multi3<Document,String, String> readParseFromNetworkCurl(String url) {
            Util.Multi3<Document, String, String> multi = readParseCurl(url, false);
            return multi == null ?  new Util.Multi3<>(null,null,null) : multi;}}
    static String writeCache(String url, String read, String redirection) {
        if (WRITE_CACHE_ENABLED.check()) { String fileName = WebReader.CACHE_FOLDER + getTimestamp() + ".html";
            write(fileName, read);
            ...
            appendSafe(REDIRECTION_FILE_SIMPLE_READ, url + DELIMITER + redirection); return fileName;}
        return null;}
    static Util.Multi3<String,String,String> read(String url){     // --------------- SIMPLE READ and NO PARSING -----------------
        synchronized (url) {
            try { if (READ_CACHE_ENABLED.check()) {
            ...
            } catch (Exception e) {return new Util.Multi3<>(null, null, null);}}}
    static String readInternal(String url, boolean preserveNewLines, Charset charset, List<String> redirectedUrls) throws IOException {
        String host;
        try { String url_clean = WebUtils.convertSpecialCharacters(url);
            host = new URI(url_clean).getHost();
        } catch (URISyntaxException e) {host  = "SYNCHRONIZED_NULL";}
        synchronized (host) {
            applyThrottle(host);
    ...
    static String readInternalPostThrottle(String url, boolean preserveNewLines, Charset charset, List<String> redirectedUrls) throws IOException {
        ...
        if (redirect) { String newUrl = conn.getHeaderField("Location");
            if (redirectedUrls == null) redirectedUrls = list();
            redirectedUrls.add(url);
            if (redirectedUrls.size() > 10){ sout("TOO MANY REDIRECTS : ");
                for (String redirected_url : redirectedUrls) {sout(redirected_url);}
                return null;}
            return readInternalPostThrottle(newUrl, preserveNewLines, charset, redirectedUrls);
        } else { String read = read(conn, preserveNewLines, charset);
            if (read != null && read.contains("charset=iso-8859-1") && charset == null){read = readInternalPostThrottle(url, preserveNewLines, StandardCharsets.ISO_8859_1, redirectedUrls);}
            return read;}}
    static Util.Multi3<Document,String,String> readParse(String url){     // --------------- SIMPLE READ and PARSING  ----------
        ...
        synchronized (url) {
            try { String url_clean = WebUtils.convertSpecialCharacters(url);
                URI uri = null;
                try { uri = new URI(url_clean);
                    url = uri.toString();
                } catch (URISyntaxException e) {url = url_clean;}
                if (MEMORY_CACHES_ENABLED.check()) {
                    if (MEMORY_CACHE_SIMPLE.containsKey(url)) {return MEMORY_CACHE_SIMPLE.get(url);}}
                if (READ_CACHE_ENABLED.check()) { String cached_location = cachedLocation(url);
                    String redirection = cachedRedirection(url);
                    if (cached_location != null && redirection != null) {
                        if (OUTPUT_CACHE_LOCATIONS) sout(url + "\n" + cached_location);
                        String html = Util.readString(cached_location);
                        if (html != null) { Document parse = Jsoup.parse(html);
                            ...
                            if (MEMORY_CACHES_ENABLED.check()) {MEMORY_CACHE_SIMPLE.put(url, parse_url);}
                            return parse_url;}}}
                ...
                if (WRITE_CACHE_ENABLED.check() && TEXT_CACHE_ENABLED && parse_url.a != null) { Util.write(file_name__text, parse_url.a.text());
    ...
    static Util.Multi3<String,String,String> headless(String url, String click_element, boolean delay){
        if (webkit == null){webkit = BrowserFactory.getBrowser(BrowserType.WebKit);}
        ...
        if (click_element != null){ String queryString;
            if (click_element.contains(":")){ String[] split = click_element.split(":");
                queryString = split[0] + "[" + split[1] + "=\"" + split[2] + "\"]"; sout(queryString);
            } else { queryString = "." + click_element;}
            Optional<io.webfolder.ui4j.api.dom.Element> query = document.query(queryString);
            if (query.isPresent()){query.get().click();}}
        if (delay){
            try {Thread.sleep(5000);
            } catch (InterruptedException ignored){}}
        String outerHTML = (String) page.executeScript("document.body.outerHTML;");
        String location = url;
        try {location = page.getWindow().getLocation();
        } catch (Exception ignored){}
        String fileName = null;
        if (outerHTML != null) {
            if (WRITE_CACHE_ENABLED.check()) { fileName = WebReader.CACHE_FOLDER_HEADLESS + getTimestamp() + ".html";
    ...
    static Util.Multi3<Document, String, String> readParseCurl(String url, boolean useReadCacheDirectinvocation){
        try { String url_clean = WebUtils.convertSpecialCharacters(url);
            ...
            if (MEMORY_CACHES_ENABLED.check()) {
                if (MEMORY_CACHE_CURL.containsKey(url)) { return MEMORY_CACHE_CURL.get(url);}}
            if (READ_CACHE_ENABLED.check() && useReadCacheDirectinvocation) { String cachedLocation = cachedLocationCurl(url);
                if (cachedLocation != null) { String html = Util.readString(cachedLocation);
                    boolean nonstandardFormatLikely = new File(cachedLocation).exists() && new File(cachedLocation).length() > 0;
                    if (html != null || nonstandardFormatLikely) { Document parse = html != null ? Jsoup.parse(html) : null;
                        Util.Multi3<Document, String, String> parseRedirection = new Util.Multi3<>(parse, url, cachedLocation);
                        if (MEMORY_CACHES_ENABLED.check()) {MEMORY_CACHE_CURL.put(url, parseRedirection);}
            ...
            if (MEMORY_CACHES_ENABLED.check()){MEMORY_CACHE_CURL.put(url, parse);}
            return parse;
        } catch (Exception e) {e.printStackTrace();}
        return null;}
    static String read(HttpURLConnection conn, boolean preserveNewLines, Charset charset) throws IOException { sout("downloading " + conn.getURL());
        BufferedReader in;
        if (charset == null) { in = new BufferedReader(new InputStreamReader(conn.getInputStream()));
        } else { in = new BufferedReader(new InputStreamReader(conn.getInputStream(),charset));}
        String inputLine;
        StringBuffer html = new StringBuffer();
        while ((inputLine = in.readLine()) != null) {html.append(inputLine);
            if (preserveNewLines) html.append("\n");}
        in.close();
        if (preserveNewLines && html.length() > 0){ return html.substring(0,html.length()-1);
        } else { return html.toString();}}
    static void applyThrottle(String host){ boolean nullHost = host == null;
        Long lastThrottleLocal = THROTTLE_LAST_MAP.get(host);
        if (nullHost){ lastThrottleLocal = THROTTLE_LAST_NULL_HOST;}
        if (lastThrottleLocal == null) lastThrottleLocal = 0L;
        long currentTime = System.currentTimeMillis();
        long difference = currentTime - lastThrottleLocal;
        if (OUTPUT_THROTTLES) {sout(host + ": Throttle identified, difference = " + difference);}
        if (nullHost) { THROTTLE_LAST_NULL_HOST = currentTime;
        } else { THROTTLE_LAST_MAP.put(host, currentTime);}
        if (difference < THROTTLE_VALUE){
            if (OUTPUT_THROTTLES) {sout(host + ": Throttle Sleeping for difference " + difference);}
            try {Thread.sleep(difference);
            } catch (InterruptedException e) {e.printStackTrace();}}}
    synchronized  static String cachedRedirection(String url) {
        if (MEMORY_REDIRECTION_SIMPLE == null) { MEMORY_REDIRECTION_SIMPLE = Util.readMapSafe(REDIRECTION_FILE_SIMPLE_READ, DELIMITER);}
        return MEMORY_REDIRECTION_SIMPLE.get(url);}
    synchronized static String cachedRedirectionCurl(String url) {
        if (MEMORY_REDIRECTION_CURL == null) {MEMORY_REDIRECTION_CURL = Util.readMapSafe(REDIRECTION_FILE_CURL, DELIMITER);}
        return MEMORY_REDIRECTION_CURL.get(url);}
    synchronized static String cachedLocation(String url) {
        if (MEMORY_URL_FILE_SIMPLE_READ == null) { MEMORY_URL_FILE_SIMPLE_READ = Util.readMapSafe(MAPPING_FILE_SIMPLE_READ, DELIMITER);}
        return MEMORY_URL_FILE_SIMPLE_READ.get(url);}
    static void loadCaches() { cachedLocationCurl(""); cachedLocation(""); cachedRedirection(""); cachedRedirectionCurl("");}
    synchronized static String cachedLocationCurl(String url) {
        if (MEMORY_URL_FILE_CURL == null ) {MEMORY_URL_FILE_CURL = Util.readMapSafe(MAPPING_FILE_CURL, DELIMITER);}
    ...
    protected static Util.Multi3<Document, String, String> simple(String url){
        ...
        if (read == null || read.equals("")){ failedSimpleUrlCalls.add(url);
    ...
    static Util.Multi3<Document, String, String> curl(String url){ String host;
        try { String url_clean = WebUtils.convertSpecialCharacters(url);
            host = new URI(url_clean).getHost();
        } catch (URISyntaxException e) {host  = "SYNCHRONIZED_NULL";}
        applyThrottle(host);
        ...
        try { Process start = pb.start();
            start.waitFor();
            if (OUTPUT_CACHE_LOCATIONS) sout(url + "\n" + fileName);
            if (WRITE_CACHE_ENABLED.check()) {appendSafe(MAPPING_FILE_CURL, url + DELIMITER + fileName);} //just the mapping file as we always write file for curl
            MEMORY_URL_FILE_CURL.put(url, fileName);
            List<String> readCurl = Util.read(fileName);
            if (readCurl == null) return null;
            StringBuilder sb = new StringBuilder();
            for (String s : readCurl) {sb.append(s + "\n");}
            String read = sb.length() > 0 ? sb.substring(0, sb.length() - 1) : "";
            Document document = Jsoup.parse(read);
            if (document == null) return new Util.Multi3<>(null, null, null);
            return new Util.Multi3<>(document, url, fileName);
        } catch (Exception e){e.printStackTrace(); return new Util.Multi3<>(null, null, null);}}
    ...
    static Util.Multi<Document,String> googleChrome(String url, String googleHeadless){ File file = new File(GOOGLE_TEMP);
        if (file.exists()) {file.delete();}
        write(GOOGLE_TEMP, ""); sout("GOOGLE " + url);
        new DownloadRunnable(url, googleHeadless).run(); sout("GOOGLE COMPLETE for " + url);
        String newFileName = CACHE_FOLDER_GOOGLE + "google" + "_" + getTimestamp() + ".txt";
        if (file.exists()) {
            try { java.nio.file.Files.copy(file.toPath(), new File(newFileName).toPath());
            } catch (IOException e) {e.printStackTrace();}} sout(newFileName);
    ...
    static Util.Multi<Document,String> phantomClickWait(String url, String phantomRoot, String selector) {return phantomjsInternal(url, phantomRoot + PHANTOM_JAVASCRIPT_CLICK_WAIT, phantomRoot, selector);}
    ...
    static Util.Multi<Document,String> phantomjsInternal(String url, String javascript, String phantomRoot, String arg3){File file = new File(PHANTOM_TEMP);
        if (file.exists()) {file.delete();}
        Util.write(PHANTOM_TEMP, ""); sout("PHANTOM " + url);
        new PhantomRunnable(url, javascript, phantomRoot, arg3).run(); sout("PHANTOM COMPLETE for " + url);
        String new_file_name = CACHE_FOLDER_PHANTOM + "phantom" + "_" + getTimestamp() + ".txt";
        if (file.exists()) {
            try {java.nio.file.Files.copy(file.toPath(), new File(new_file_name).toPath());
            } catch (IOException e) {e.printStackTrace();}}
        sout(new_file_name);
        String html = Util.readString(new_file_name);
        if (html == null || html.length() == 0) return null;
        return new Util.Multi<>(Jsoup.parse(html), new_file_name);}
    static class PhantomRunnable implements Runnable { Process process = null; String url; String arg3; final String javascript; final String phantomRoot;
        PhantomRunnable(String url, String javascript, String phantomRoot, String arg3) {this.url = url; this.javascript = javascript; this.phantomRoot = phantomRoot; this.arg3 = arg3;}
        public void run() {
            try {ArrayList<String> list = list();
                String phantomExecutable = PHANTOM_EXECUTABLE;
                String javascriptlocal;
                if ("\\".equals(SLASH)) { phantomExecutable = "\"" + phantomRoot + phantomExecutable + ".exe\"";
                    javascriptlocal = "\"" + javascript + "\"";
                } else { phantomExecutable = phantomRoot + phantomExecutable;
                    javascriptlocal = javascript;}
                list.add(phantomExecutable);
                list.add(javascriptlocal);
                if ("\\".equals(SLASH)) { list.add("\"" + url + "\"");
                } else { list.add(url);
                    list.add(PHANTOM_TEMP);}
                if (!Util.empty(arg3)){list.add(arg3);}
                process = new ProcessBuilder(list).start();
                process.waitFor(15, TimeUnit.SECONDS);
            } catch (Exception ignored) {
            } finally { if (process != null) {process.destroy();}}}}
    static class DownloadRunnable implements Runnable { Process process = null; String url; final String googleHeadless;
        DownloadRunnable(String url, String googleHeadless) {this.url = url;this.googleHeadless = googleHeadless;}
        public void run() {
            try { process = new ProcessBuilder(list(googleHeadless, url, GOOGLE_TEMP)).start();
                process.waitFor(15, TimeUnit.SECONDS);
            } catch (Exception ignored) {
            } finally {if (process != null) {process.destroy();}}}}
    static class Download {
        static Util.Multi3<Document,String,String> document(Context ctx, String url, BaseDirs dirs, boolean delay) {return document(ctx, url, null, dirs, delay);}
        static Document documentOnly(Context ctx, String url, BaseDirs dirs, boolean delay){return safeA(documentRedirect(ctx, url, dirs, delay));}
        static Util.Multi<Document, String> documentRedirect(Context ctx, String url, BaseDirs dirs, boolean delay){
            Util.Multi3<Document, String, String> multi = document(ctx, url, dirs, delay); return new Util.Multi<>(safeA(multi), safeB(multi));}
        static Util.Multi3<Document, String, String> document(Context ctx, String url, String clickElement, BaseDirs dirs, boolean delay) {Util.Multi3<Document,String,String> multi;
            if (ctx.downloadCurl()){ multi = Advanced.readParseFromNetworkCurl(url); if (WebUtils.safeA(multi) == null) {sout("CURL Download Failed for url " + url);}
            } else if (ctx.downloadPhantom()){ Util.Multi<Document,String> documentLocation = WebReader.phantomjs(url, dirs.getPhantomDir(), delay);
                multi = new Util.Multi3<>(Util.Multi.safeA(documentLocation), url, Util.Multi.safeB(documentLocation));
            } else if (ctx.downloadGoogle()){ Util.Multi<Document,String> docLocation = googleChrome(url, dirs.getGoogleScript());
                multi = new Util.Multi3<>(Util.Multi.safeA(docLocation), url, Util.Multi.safeB(docLocation));
            } else { Util.Multi3<String, String, String> webmulti = headless(url, clickElement, delay); //delay
                if (WebUtils.safeA(webmulti) == null) { multi = new Util.Multi3<>(null, null, null);sout("UI4J Download Failed" + (!empty(clickElement) ? " with click element " + clickElement : "") + " for url " + url);
                } else { multi = new Util.Multi3<>(Jsoup.parse(webmulti.a), webmulti.b, webmulti.c);}}
            return new Util.Multi3<>(WebUtils.safeA(multi),WebUtils.safeB(multi), WebUtils.safeC(multi));}
        static Util.Multi<Document, String> phantomjs(String link, Context ctx, BaseDirs dirs, boolean delay) { String clickWaitSelector = ctx.downloadClickWait();
            if (ctx.downloadScroll()){return phantomScroll(link, dirs);
            } else if (!empty(clickWaitSelector)){return phantomClickWait(link, dirs, clickWaitSelector);
            } else {return phantomjs(link, delay, dirs);}}
        ...
        static Util.Multi<Document, String> phantomClickWait(String link, BaseDirs dirs, String selector) {
            ...
            return new Util.Multi<>(Util.Multi.safeA(documentLocation), Util.Multi.safeB(documentLocation));}}
    static class UriExtension { static List<String> specialUrls = list();
        static Map<String, String> urlHost = map();
        static boolean indexHtmlCheck = false;
        static String toFullUrl(String baseUrl, String uri) { if (uri == null) return null;
            if (uri.equals("/")) return baseUrl;
            if (urlHost.get(baseUrl) == null){
                try {urlHost.put(baseUrl, new URI(WebUtils.convertSpecialCharacters(baseUrl)).getHost());
                } catch (Exception e){e.printStackTrace();return null;}}
            uri = uri.trim();
            String uriToAdd = null;
            String host = urlHost.get(baseUrl);
            if (uri.startsWith("http:") || uri.startsWith("https:")){ uriToAdd = uri;
            } else if (uri.startsWith("//")){ uriToAdd = "http:" + uri;
            } else if (uri.startsWith("/")) {
                if (host.endsWith("/")) {uri = uri.substring(Math.max(uri.length(),1));}
                uriToAdd = host + uri;
                if (baseUrl.startsWith("https:")) {
                    if (uriToAdd.startsWith("www.")) { uriToAdd = "https://" + uriToAdd;
                    } else {uriToAdd = uriToAdd.replaceFirst("http:", "https:");}}
                if (baseUrl.startsWith("http:")) {
                    if (uriToAdd.startsWith("www.")) { uriToAdd = "http://" + uriToAdd;
                    } else {uriToAdd = uriToAdd.replaceFirst("https:", "http:");}
                }} else if (uri.startsWith("?")){uriToAdd = baseUrl + uri;
            } else if (uri.startsWith("#/")){
                if (baseUrl.contains("#/")){ String[] split = baseUrl.split("#/");
                    uriToAdd = "";
                    for (int i = 0; i < split.length - 1; i++) {uriToAdd = uriToAdd + "#/" + split[i];}
                    uriToAdd = uriToAdd + uri;
                } else {uriToAdd = baseUrl + uri;
                }} else if (!uri.startsWith("mailto:")){
                if (baseUrl.endsWith(".html") && indexHtmlCheck){ String[] split = baseUrl.split("/");
                    StringBuilder sb = new StringBuilder();
                    for (int i = 0; i < split.length - 1; i++) {sb.append(split[i] + "/");}
                    sb.append(uri);
                    uriToAdd = sb.toString();
                } else if (baseUrl.endsWith(".html")) { uriToAdd = host + (host.endsWith("/") ? "" : "/") + uri;
                } else {String slash = baseUrl.endsWith("/") ? "" : "/";
                    for (String specialUrl : specialUrls) {
                        if (baseUrl.contains(specialUrl)){uriToAdd = specialUrl + slash + uri;}}
                    if (uriToAdd != null){
                    } else if (baseUrl.contains("?")){String[] split = baseUrl.split("\\?")[0].split("/");
                        StringBuilder sb = new StringBuilder();
                        for (int i = 0; i < split.length - 1; i++) {sb.append(split[i] + "/");}
                        sb.append(uri);
                        uriToAdd = sb.toString();
                    } else { uriToAdd = baseUrl + slash + uri;}
                }} else {
                uriToAdd = uri;}
            if (!uriToAdd.startsWith("http") && !uri.startsWith("mailto:")){uriToAdd = "http://" + uriToAdd;}
            return uriToAdd;}}}