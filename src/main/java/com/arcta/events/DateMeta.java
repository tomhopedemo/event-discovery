...
class DateMeta {List<Calendar.Date> between;     // ----- DOUBLE DATE RETURN STRUCTURES ---------
    ...
    static boolean empty(DateMeta date_internal) {if (date_internal == null) return true;
        return (Util.empty(date_internal.dateList) && Util.empty(date_internal.between) && Util.empty(date_internal.betweenList));}
    static List<Calendar.Date> dateList(DateMeta date_internal){ if (date_internal == null) return null;
