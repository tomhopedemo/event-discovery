...
class BetweenTwoDatesText extends DateMatcher {final boolean allIndices;
    BetweenTwoDatesText(){
        this(false);
    } BetweenTwoDatesText(boolean allIndices){
        this.allIndices = allIndices;
    }
    public DateMeta matchInternal(String text) { String timeO = "([0-9]{1,2}:[0-9]{2})?";
        ...
        while (matcher.find()){
            Calendar.Date dateFrom = new Calendar.Date(); Calendar.Date dateTo = new Calendar.Date();
            dateTo.dateDay = matcher.group(11).trim();
            if (dateTo.dateDay.length() == 2 && dateTo.dateDay.startsWith("0")) {dateTo.dateDay = dateTo.dateDay.substring(1,2);}
            ...
            if (dateFrom.dateDay.length() == 2 && dateFrom.dateDay.startsWith("0")) {dateFrom.dateDay = dateFrom.dateDay.substring(1,2);}
            ...
            if (matcher.group(7) != null){ dateFrom.dateYear = "20" + matcher.group(7).trim();
            } else {
                if (MONTHS_ORDER.indexOf(dateFrom.dateMonth) <= MONTHS_ORDER.indexOf(dateTo.dateMonth)){ dateFrom.dateYear = dateTo.dateYear;
                } else { dateFrom.dateYear = String.valueOf(Integer.parseInt(dateTo.dateYear) -1);}}
            ...
            if (midIndexA == null) { getIndexPairs(text, meta, matcher.start(), matcher.end());
            } else { getIndexPairs(text, meta, matcher.start(), midIndexA);
            ...
            for (Calendar.Date date : dates) {
                ...
                if (midIndexA == null) { indexPairs.add(new Util.Multi<>(matcher.start(), matcher.end()));
                } else {
        ...
        meta.between = Util.get(meta.betweenList,0); return meta;}}                // 1 apr 2020 - 2 apr 2020