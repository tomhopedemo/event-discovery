...
class SingleTime extends TimeMatcher {boolean pm; boolean doors;
    SingleTime(boolean pm, boolean doors) {this.pm = pm; this.doors = doors;}
    ...
    List<Time> match(Util.StringMutable clean, boolean exact_match) {
        ...
        while (matcher.find()) {
            ...
            if (doors && "doors".equals(matcher.group(1))) { hourInt = hourInt + 1;}
            ...
            if (Util.list("time", "time ").contains(matcher.group(1))) { timeIndicative.convertTo24H(8);
            } else { timeIndicative.convertTo24H();}
