...
class London { static Util.Table londonAreaConfig;
    static final Map<String, List<String>> SUBPARTS = Util.map();
    final static String LONDON_MAP =
            ",,,,,,Highgate,,,Harringay,,,,,\n" +
                    ...
                    ",,,,,Balham,Streatham,,,,,,,,\n" +
                    ",,,,,,,,,Nunhead,,,,,";
    static { SUBPARTS.put("north london", list("finsbury park")); SUBPARTS.put("south london", list("elephant")); SUBPARTS.put("west london", list("hammersmith")); SUBPARTS.put("east london", list("Whitechapel")); SUBPARTS.put("hackney", list("hoxton", "haggerston", "shoreditch")); SUBPARTS.put("aldwych", list("fleet street")); SUBPARTS.put("bank", list("leadenhall market")); SUBPARTS.put("the city", list("liverpool street", "bank", "aldgate")); SUBPARTS.put("victoria", list("belgravia", "westminster", "pimlico")); SUBPARTS.put("oxford street", list("marylebone", "fitzrovia", "soho")); SUBPARTS.put("trafalgar square", list("covent garden", "piccadilly", "westminster"));  SUBPARTS.put("harringay", list("manor house")); SUBPARTS.put("tuffnell park", list("archway")); SUBPARTS.put("cannon street", list("bank")); SUBPARTS.put("monument", list("bank")); SUBPARTS.put("ladbroke grove", list("notting hill", "north kensington")); SUBPARTS.put("guildhall", list("st pauls", "liverpool street", "bank")); SUBPARTS.put("temple", list("aldwych")); SUBPARTS.put("leicester square", list("soho", "piccadilly", "covent garden")); SUBPARTS.put("hyde park corner", list("hyde park", "mayfair", "belgravia")); SUBPARTS.put("embankment", list("covent garden"));  SUBPARTS.put("strand", list("covent garden", "aldwych"));  SUBPARTS.put("fleet street", list("aldwych", "st pauls")); SUBPARTS.put("sloane square", list("victoria", "belgravia")); SUBPARTS.put("manor house", list("harringay", "stoke newington"));  SUBPARTS.put("oval", list("kennington"));
    ...
    static List<String> areasDisplay(){ ArrayList<String> to_return = new ArrayList<>(London.areasRestrict);
        to_return.removeAll(FAROUT);
        to_return.removeAll(ODD); return to_return;}
    static Util.MultiList<Double, String> areasOrderedByProximity(String area){ List<String> areas;
        if (SUBPARTS.containsKey(area)){ areas = SUBPARTS.get(area);
            areas.add(area);
        } else { areas = list(area);}
        Util.MultiList<Integer, Integer> areaCoordinates = new Util.MultiList<>();
        for (String areaLocal : areas) {Util.addAll(areaCoordinates, londonAreaConfig.getIgnoreCase(areaLocal));}
        ...
        for (int i = 0; i < londonAreaConfig.underlying.size(); i++) { List<String> row = londonAreaConfig.underlying.get(i);
            for (int j = 0; j < row.size(); j++) { String element = row.get(j);
                if (Util.empty(element)) continue;
                if (area.equalsIgnoreCase(element)) continue;
                Double distance_min = null;
                for (Util.Multi<Integer, Integer> coordinate : areaCoordinates.underlying) { double distance = distanceAbsolute(coordinate, new Util.Multi<>(i,j));
                    if (distance_min == null || distance < distance_min){ distance_min = distance;}}
                distance_area.add(new Util.Multi<>(distance_min, element));}}
        Collections.sort(distance_area.underlying, Comparator.comparing(o -> o.a)); return distance_area;}
    static double distanceAbsolute(Util.Multi<Integer, Integer> coordinate_a, Util.Multi<Integer, Integer> coordinate_b){return Math.sqrt(Math.pow(coordinate_a.a - coordinate_b.a, 2) + Math.pow(coordinate_a.b - coordinate_b.b, 2));}}