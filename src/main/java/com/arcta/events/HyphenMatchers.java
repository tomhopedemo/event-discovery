...
class HyphenMatchers {
    ...
    static { StringBuilder sbHyphen = new StringBuilder();
        ...
        for (String hyphen : Util.HYPHENS) { sbHyphen.append(hyphen + "|");}
        ...
        for (String symbol : hyphensAndUnderscoresAndDots){ sbHyphUnder.append(symbol + "|");}
        ArrayList<String> hyphensAndTo = new ArrayList<>(Util.HYPHENS);
        hyphensAndTo.add("to");
        for (String symbol : hyphensAndTo){sbHyphTo.append(symbol + "|");}
        ...
        for (String symbol : hyphensAndToUntil){sbHyphToUntil.append(symbol + "|");}
