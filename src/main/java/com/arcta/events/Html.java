...
class Html {
    ...
    Map<Integer, DisplayEvent> execute(HtmlContext ctx) {
        ...
        for (int plusDays = 0; plusDays < numdays; plusDays++) {
            CalendarDate cd = Calendar.get(currentIndex + 1 + plusDays);
    ...
    boolean writeHtmls(HtmlContext ctx, boolean indexWritten, int plusDays, Calendar.Date date, String weekday, Util.Multi<String, DisplayEvent> htmlMain, Util.PageData pageData, String year) {
    ...
    boolean isBankHoliday(Calendar.Date date) {
...
//            sb.append(menu);
    ...
    static class HtmlHelper {
        ...
        static List<CalendarDate> nextDays(CalendarDate baseDate) {
            int i = Calendar.indexOf(baseDate);
            List<CalendarDate> toReturn = list();
            for (int j = 0; j < 31; j++) {
                toReturn.add(Calendar.get(j + i));
            }
            return toReturn;
        }
