...
class DateTimeParse {
    static Map<Calendar.Date, Time> make(Document doc, Context ctx){  String identifier = ctx.linkxClass();
        Elements elements = doc.getElementsByClass(identifier);
        Map<Calendar.Date, Time> dateTimes = map();
        for (Element element : elements) { Util.Multi<String, Util.MultiList<Integer, Element>> textHtml = Jutil.text(element);
            ...
            Calendar.Date date = match.get(0); if (date == null) continue;
            Time time = TimeMatcher.match(clean, ctx.lang, ctx.time); if (time == null) continue;
