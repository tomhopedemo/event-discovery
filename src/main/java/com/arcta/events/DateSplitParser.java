...
class DateSplitParser {
    ...
    static void make(Context ctx, Util.Url url, Document doc, Dirs dirs) {
        ...
        if (!Util.empty(aftertext)){
            ...
            if (split.size() > 1){text = split.get(1);}}
        ...
        if (!Util.empty(ctx.partialTag())) { List<Calendar.Date> singledates = DateMatcher.matchNoOverlapsPartial(text, ctx.lang, ctx.date, url.date);
            if (Util.empty(singledates)) return;
            dateLists = Util.list();
            for (Calendar.Date singledate : singledates) { dateLists.add(Util.list(singledate));
            }} else { dateLists = DateMatcher.matchNoOverlaps(text, ctx.lang, ctx.date);}
        ...
        for (int i = 0; i < dateLists.size(); i++) { List<Calendar.Date> dates = dateLists.get(i);
            Integer index1; Integer index2; Integer indexX;
            if (ctx.datesplitReverse()){ index1 = i == 0 ? 0 : dateLists.get(i - 1).get(0).indexPairs.get(0).b;
                indexX = index1; //unimplemented
                index2 = dates.get(0).indexPairs.get(0).b;
            } else { index1 = dates.get(0).indexPairs.get(0).a;
            ...
            if (ctx.datesplitNoTime()) { Event event = Parse.eventManufacture(ctx, url.url, clean, dates);
                if (event == null) continue;
                String link = generateLink(index1, index2, indexElement, url.url);
                if (Util.empty(event.dateTimes) && link != null){
                    if (!Util.empty(ctx.timeSkip())){ Time timeSkiptime = TimeMatcher.matchSingle(Util.lowercase(ctx.timeSkip()), false, ctx.lang, ctx.time);
                        dates.forEach(d -> event.dateTimes.put(d, timeSkiptime));
                    } else if (!ctx.parseNolink()) { event.dateTimes = EventLink.make(link, ctx, dates, dirs.getBaseDirs());}}
                if (link != null){ event.link = link;
                    Map<Calendar.Date, String> newDateUrlMap = map();
                    for (Calendar.Date date : Util.safeNull(event.dateUrl.keySet())) {newDateUrlMap.put(date, event.link);}
                    event.dateUrl = newDateUrlMap;}
                REF_EVENTS.put(ctx.ref, event);}}
        if (ctx.datesplitNoTime()) return;
        for (int j = 0; j < splitTextDate.size(); j++) { Util.Multi<String,List<Calendar.Date>> dateMulti = splitTextDate.underlying.get(j);
            String datePart = dateMulti.a;
            List<Time> times = TimeMatcher.matchNoOverlaps(datePart, ctx.lang, ctx.time);
            for (int i = -1; i < times.size(); i++) { Integer index1 = i == -1 ? 0 : times.get(i).startIndex;
                ...
                if (urlIndicative != null) { event.link = urlIndicative;
                    Set<Calendar.Date> keys = Util.safeNull(event.dateUrl.keySet());
                    for (Calendar.Date date : keys) {event.dateUrl.put(date, urlIndicative);}}
                REF_EVENTS.put(ctx.ref, event);}}}
    static Map<String, String> constructATextHrefMap(Element doc) {if (doc == null) return null;
    ...
    static String generateLink(Map<String,String> textHref, String originalSubstring, String url){
        if (textHref != null) { String longestMatch = null;
            for (String textlet : textHref.keySet()) {
                ...
                if (longestMatch == null) {longestMatch = textlet;
                } else if (longestMatch.length() < textlet.length()) {longestMatch = textlet;}}
    ...
    static String generateLink(Integer index1, Integer index2, Util.MultiList<Integer, Element> indexHrefs, String url) {
        ...
        for (Util.Multi<Integer, Element> indexHref : indexHrefs.underlying) {
            if (Util.between(indexHref.a, index1, index2)){ Element b = indexHref.b;
                if (b != null && b.tag() != null){
                    if ("a".equals(b.tag().toString())) { hrefCounts.add(indexHref.b);}}}}
        hrefCounts.greaterThan(1);
        if (hrefCounts.map.size() > 0){ String href = hrefCounts.getLargest().attr("href");
            return WebReader.UriExtension.toFullUrl(url, href);}
        return null;}}