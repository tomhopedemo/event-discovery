...
import static com.arcta.events.FiltersLists.REPEAT_PHRASES_ALLOWED;
import static com.arcta.events.Util.map;
import static com.arcta.events.Util.sout;
class Replica {
    static void cleanReplicaWords(Util.StringMutable clean, String url, Context.LanguageContext langCtx){ if (Util.empty(clean)) return;
        String replaced = clean.string.replaceAll("\u2021"," ");
        String urlend = "";
        if (url != null){ urlend = Util.last(Util.splitList(url, "/"));
    ...
    static void reapplyUrlTriplet(Util.StringMutable clean, String urlend, String original) { List<String> strings = PhraseUtil.wordList(urlend);
        if (Util.empty(strings)) return;
        String x = strings.get(0);
        int count = 0;
        boolean found = false;
        for (int i = 0; i < strings.size(); i++) {
            if (strings.get(i).equals(x)){ count++;
            } else { x = strings.get(i);
                count = 0;}
            if (count == 3) { found = true; break;}}
        String triplet = x + " " + x + " " + x;
        if (found && original.contains(triplet)){ clean.string = clean.string.replaceFirst(Pattern.quote(x), triplet);
            sout("TRIPLETXXX: " + clean.string);}}
    static List<Integer> replicaPhraseIndices(String string, int phraseLength, Context.LanguageContext langCtx) {         List<Integer> indexesToRemove = Util.list();
        ...
        for (int i = 0; i < words.size() - (phraseLength -1); i++) {
            ...
            for( int j = 0; j < phraseLength; j++) {list.add(words.get(i + j));}
            ...
            if (values.containsKey(phrase)) {
                ...
                values.put(phrase, new Util.Multi<>(startInclusive, endInclusive));}} return indexesToRemove;}
    static void cleanRepeatWord(Util.StringMutable clean){         List<Integer> indicesToExclude = Util.list();
        Util.NList split = Util.split(clean.string, "\\s+");
        if (split.size() < 2) return;
        for (int i = 0; i < split.size() -1; i++) { String a = PhraseUtil.stripPunctuation(split.get(i));
